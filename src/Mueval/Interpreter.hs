...
printInterpreterError (WontCompile errors) =
    ...
    do
        sayIO $ concatMap (dropLinePosition . errMsg) errors
        exitFailure
  where
...
render i xs =
    do
        (out, Any b) <- runWriterT $ render' i (toStream xs)
        return (out, b)
  where
...
toStream str =
    E.evaluate (uncons str)
        `E.catch` \(E.SomeException e) -> return . Exception . toStream . show $ e
  where
    uncons [] = End
